...
message('loading pubmed.txt')
# export to 'pubmed.txt' using Save >> PubMed on pubmed.gov
slurpFile <- function(fileName) readChar(fileName, file.info(fileName)$size)
importFile <- 'pubmed.txt'
if(!file.exists(importFile)) q('no')
d <- slurpFile(importFile)
d <- gsub('\\r', '', d)
d <- gsub('\\n      ', ' ', d)
d <- strsplit(d, '\\n\\n')[[1]] # result is one text block per citation

# load known publications already entered into the site
message('loading pre-existing publications')
publicationsYml <- '_data/publications.yml'
publications <- read_yaml(publicationsYml)
...
message('importing/updating publications')
...
for(i in seq_along(d)){
    ...
    if(df[i, 'PMID'] %in% names(pmids)){
        ...
        for(field in c('date', 'pmcid', 'authors', 'title', 
            ...
            publications[[j]][[field]] <- incoming[[field]]
        } 
    } else {
...
archiveFile <- paste0('_data/_archive/publications-', Sys.Date(), '.yml')
...
write_yaml(publications, publicationsYml)
...
#  $ AU  : chr "Moran JV"
#  $ AD  : chr "Department of Human Genetics, University of Michigan Medical School, Ann Arbor,  Michigan, 48109, USA; Departme"| __truncated__
#  $ LA  : chr "eng"
...
#  $ COIS: chr "DECLARATION OF INTERESTS JVM is an inventor on patent US6150160, is a paid  consultant for Gilead Sciences and "| __truncated__
